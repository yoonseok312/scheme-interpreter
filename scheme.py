...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        new_rest = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operator, new_rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        first_value = scheme_eval(expressions.first, env, True)
        return first_value
    else:
        first_value = scheme_eval(expressions.first, env)
        if scheme_truep(first_value):  
            return do_and_form(expressions.rest, env)
        elif scheme_falsep(first_value): 
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal_param = Pair(bindings.first.first, formal_param)
        value = Pair(scheme_eval(bindings.first.rest.first, env), value)
        bindings = bindings.rest
        check_formals(formal_param)
    return env.make_child_frame(formal_param, value)

def do_define_macro(expressions, env):
    """Evaluate a define-macro form."""
    check_form(expressions, 2, 2)
    operand = expressions.first
    if not isinstance(operand, Pair) or not scheme_symbolp(operand.first):
        raise SchemeError
    label, formals = operand.first, operand.rest
    body = expressions.rest
    env.define(label, MacroProcedure(formals,body,env))
    return label
